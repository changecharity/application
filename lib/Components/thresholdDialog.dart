...
class _ThresholdDialogState extends State<ThresholdDialog> with TickerProviderStateMixin{
  ...
  void _setInitial() {
    setState(() {
      _sliderController.value = (widget.initialVal * 2 - 100) / 100 + (widget.initialVal == 50 ? 0.01 : 0);
  ...
  void _setThreshold() async {
    ...
    print(threshold);
    context.read<UserProfileModel>().notify(context.read<UserProfileModel>().getUserPfLetter, context.read<UserProfileModel>().getUserName, threshold.toInt(), context.read<UserProfileModel>().getUserMonthlyLimit, context.read<UserProfileModel>().getUserRoundUpStatus);
  }
}