...
//          subtitle: Text('Donor since August 2008'),
    ...
    return Consumer<UserBankModel>(
      builder: (context, userBank, child) {
        return ListTile(
          ...
          subtitle: userBank.getBankName == null ? Text("Link your Payment Method" , style: TextStyle(color: Colors.redAccent),) : Text("${userBank.getBankName} ...${userBank.getMask}"),
          onTap: () {
            userBank.getBankName == null ?
            Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) => LinkCredit())) : Navigator.of(context).push(new MaterialPageRoute<Null>(
                builder: (BuildContext context) {
  ...
  Widget _roundUpText() {
    ...
    if (bankName  == null) {
      return Text("Link your Payment Method", style: TextStyle(color: Colors.redAccent),);
    } else if (length == 0) {
      return Text("Link your Round Up Account", style: TextStyle(color: Colors.redAccent),);
    } else {
      return Text("$length connected accounts");
    }
  }
  ...
  void _roundUpAction() {
    ...
    if (bankName == null) {
      ...
      Navigator.of(context).push(new MaterialPageRoute<Null>(builder: (BuildContext context) {return BankAccountsDialog();}, fullscreenDialog: true));
    }
  }
    ...
    return Consumer<UserProfileModel>(
      builder: (context, userPF, child) {
        return ListTile(
          ...
          subtitle: Text("${Money.fromInt(userPF.getUserThreshold, usdCurrency)}"),
          onTap: () {
            Navigator.of(context).push(new MaterialPageRoute<Null>(
                builder: (BuildContext context) {
    ...
    return Consumer<UserProfileModel>(
      builder: (context, userPF, child) {
        return ListTile(
          ...
          subtitle: Text(userPF.getUserMonthlyLimit == 0 ? "Turned off" : "${Money.fromInt(userPF.getUserMonthlyLimit, usdCurrency)}"),
          onTap: () {
            Navigator.of(context).push(new MaterialPageRoute<Null>(
                builder: (BuildContext context) {
